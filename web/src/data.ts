// Sample content for the Pulse app. Mirrors the design-system sample data.
...
export interface PulseEvent {
  ...
  relevance: string[];
  sources: string[];
  /** background-position for the duotone placeholder (e.g. "18% 30%") */
  img?: string;
  imageStyle?: CSSProperties;
  /** real photo URL from a source (Ticketmaster/SeatGeek/JSON-LD); duotone fallback when absent */
  imageUrl?: string;
  /** outbound link to the source (for "Get tickets") */
  url?: string;
  /** real coordinates (for the map projection) */
  lat?: number;
  lng?: number;
  /** ISO start datetime (for sorting by soonest) */
  start?: string;
  blurb: string;
}
...
export interface City {
  ...
  lat?: number;
  lng?: number;
  region?: string; // state/province for disambiguating same-named cities
}
...
export const interests: string[] = [
  "Live music", "Art", "Film", "Food", "Theatre", "Talks",
  "Comedy", "Markets", "Workshops", "Clubs", "Books", "Design",
];
