...
type Glyph = "map" | "search" | "saved" | "quiet" | "error";
 ...
 */
export function EmptyState({ title, body, action, glyph = "map", style }: EmptyStateProps) {
  return (
    <div
    ...
    >
      ...
      <h3 style={{ fontFamily: "var(--font-display)", fontSize: "var(--fs-display-sm)", color: "var(--text-strong)", margin: 0 }}>
        {title}
      </h3>
      {body && (
        <p style={{ fontSize: "var(--fs-subtitle)", color: "var(--text-muted)", lineHeight: "var(--lh-normal)", margin: 0 }}>
          {body}
        </p>
      )}
