...
export function DatePicker({ value, onChange, style }: DatePickerProps) {
  ...
  return (
    <div
      style={{
        width: "280px", background: "var(--surface-card)", border: "1px solid var(--border)",
        borderRadius: "var(--radius-lg)", padding: "var(--space-4)", boxShadow: "var(--shadow-3)",
        ...style,
      }}
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "var(--space-3)" }}>
        <span style={{ fontFamily: "var(--font-display)", fontSize: "var(--fs-subtitle)", color: "var(--text-strong)", fontWeight: "var(--fw-semibold)" }}>
          {monthName}
        </span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "2px", marginBottom: "4px" }}>
        {["M", "T", "W", "T", "F", "S", "S"].map((d, i) => (
          <span key={i} style={{ textAlign: "center", fontFamily: "var(--font-mono)", fontSize: "var(--fs-2xs)", letterSpacing: "0.04em", color: "var(--text-faint)", padding: "4px 0" }}>
            {d}
          </span>
        ))}
      </div>
