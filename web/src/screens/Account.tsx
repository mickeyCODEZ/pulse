...
export interface AccountProps {
  ...
  onSignOut?: () => void; // unused (anonymous app); kept for caller compatibility
  city: City;
}

export function Account({ theme, setTheme, onNavigate, city }: AccountProps) {
  ...
  return (
    <div style={{ padding: "var(--space-6)", maxWidth: "var(--content-max)", margin: "0 auto" }}>
      {/* identity — anonymous, honest */}
      <div style={{ display: "flex", alignItems: "center", gap: "var(--space-4)", marginBottom: "var(--space-6)" }}>
        <Avatar name="You" size={60} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "var(--fs-display-sm)", color: "var(--text-strong)", margin: "0 0 2px", letterSpacing: "var(--ls-tight)" }}>
            You
          </h1>
          <div style={{ fontFamily: "var(--font-mono)", fontSize: "var(--fs-xs)", color: "var(--text-muted)" }}>
            Browsing anonymously — saves live on this device
          </div>
        </div>
